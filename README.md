...
roundFileInput.addEventListener('change', (ev)=>{
  ...
  const countdownActive = countBadge && countBadge.style.display !== 'none';
  if (countdownActive) {
    ensureAudio();
    roundAudioEl.play().then(()=>{
...
muteSoundBtn.addEventListener('click', ()=>{
  if(roundAudioEl && !roundAudioEl.paused){
    roundAudioEl.muted = !roundAudioEl.muted;
    muteSoundBtn.textContent = roundAudioEl.muted ? "🔈 Reativar" : "🔇 Silenciar";
  }else{
    if(synth.running){
      stopSynth();
      muteSoundBtn.textContent = "🔈 Reativar";
    }else{
      startSynth();
      muteSoundBtn.textContent = "🔇 Silenciar";
    }
  }
});
function startCountdownBadgeUI(data){
  ...
  const tryPlay = async () => {
    ...
    if (roundAudioEl && roundAudioEl.src) {
      ...
      catch(e){ playSoundBtn.style.display='inline-block'; muteSoundBtn.style.display='none'; }
    } else {
      startSynth();
    }
  };
...
genEmbedHtml.onclick = async ()=>{
  ...
  a.download = 'dobble_v36_embutido.html';
